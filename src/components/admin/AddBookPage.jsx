...
function AddBookPage() {
  ...
  const handleSubmit = () => {
    if (inputData.title === "" && inputData.title.length <= 0) {
      handleShowSnackbar("Title is required.", "warning", "title_field_error");
    } else if (inputFile === null) {
      handleShowSnackbar("File is required.", "warning", "file_field_error");
      ...
      dispatch(addBook(auth.currentUser.uid, inputFile, _data))
        ...
        .catch((err) => {
          handleShowSnackbar(err.message, "error", "upload_error");
        });
    }
  };
